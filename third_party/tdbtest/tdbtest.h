...
template <class T> std::string ToDebugString(const T& value) {
...
template <class A, class B> bool ValuesEqual(const A& a, const B& b) {
...
void ExpectEq(const A& actual, const B& expected, const char* actual_str, const char* expected_str, const char* file,
			  int line) {
	if (!ValuesEqual(actual, expected)) {
		ReportFailure(file, line,
					  std::string("Expected equality of ") + actual_str + " and " + expected_str +
...
template <class A, class B>
void ExpectNe(const A& actual, const B& expected, const char* actual_str, const char* expected_str, const char* file,
			  int line) {
	if (ValuesEqual(actual, expected)) {
...
inline void ExpectNear(double actual, double expected, double abs_error, const char* actual_str,
	...
	if (std::fabs(actual - expected) > abs_error) {
		ReportFailure(file, line,
					  std::string("Expected near: ") + actual_str + " vs " + expected_str +
						  "\n    Actual: " + ToDebugString(actual) + "\n  Expected: " + ToDebugString(expected));
...
#define EXPECT_EQ(actual, expected) ::tdbtest::ExpectEq((actual), (expected), #actual, #expected, __FILE__, __LINE__)

#define EXPECT_NE(actual, expected) ::tdbtest::ExpectNe((actual), (expected), #actual, #expected, __FILE__, __LINE__)
...
#define EXPECT_THROW(statement, exception_type)                                                                        \
	do {                                                                                                               \
		...
		try {                                                                                                          \
			...
			::tdbtest::ReportFailure(__FILE__, __LINE__, std::string("Wrong exception type: ") + ex.what());           \
		}                                                                                                              \
		if (!caught_) {                                                                                                \
