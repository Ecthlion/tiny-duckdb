...
> 对应测试：`test/lab1_storage_test.cpp`（17 个用例）
...
**测试**：`TableDataSplitsRowGroups`（2×ROW_GROUP_SIZE+17 行 → 恰好 3 个行组）、`TableScanMorselsCoverAllRows`（morsel 列表不重不漏）、`TableScanRoundTrip`、`ConcurrentTableAppendsAreAtomic`（4 个线程并发追加后不丢行、不重复）。
