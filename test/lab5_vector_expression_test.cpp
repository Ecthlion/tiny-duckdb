...
TEST(Lab5VectorExpressionTest, BinderChecksFunctionAndDimension) {
	...
	auto bound = BindVectorSql(catalog, "SELECT cosine_distance(embedding, [1, 0, 0]) AS distance FROM items");
	...
	EXPECT_THROW(BindVectorSql(catalog, "SELECT l2_distance(embedding, [1, 0]) FROM items"), BinderException);
	EXPECT_THROW(BindVectorSql(catalog, "SELECT cosine_distance(id, [1, 0, 0]) FROM items"), BinderException);
...
TEST(Lab5VectorExpressionTest, ExactTopKCosineQuery) {
	...
	connection.Query("INSERT INTO docs VALUES "
					 "(1, 'database systems', [1, 0, 0]), "
					 "(2, 'query execution', [0.9, 0.1, 0]), "
					 "(3, 'cooking', [0, 0, 1]), "
					 "(4, 'machine learning', [0.7, 0.3, 0])");

	auto result = connection.Query("SELECT id, title, cosine_distance(embedding, [1, 0, 0]) AS distance "
...
TEST(Lab5VectorExpressionTest, L2AndInnerProductQueries) {
	...
	auto l2 = connection.Query("SELECT id, l2_distance(embedding, [0.9, 1.1]) AS distance "
	...
	auto inner = connection.Query("SELECT id, negative_inner_product(embedding, [1, 1]) AS score "
...
TEST(Lab5VectorExpressionTest, NullDistancePropagates) {
	...
	auto result = connection.Query("SELECT id, cosine_distance(embedding, [1, 0]) AS distance FROM points");
