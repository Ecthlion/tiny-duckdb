...
TEST(Lab3ExecutionTest, WhereConjunction) {
	...
	EXPECT_EQ(all->GetValue(0, 0),
			  result->ToRows().empty() ? Value::BigInt(0) : Value::BigInt(static_cast<int64_t>(result->RowCount())));
}
...
TEST(Lab3ExecutionTest, JoinGroupBy) {
	TinyDuckDB db;
	CreateOrdersAndCustomer(db);
	auto result = Run(db, "SELECT c_name, count(*) FROM orders JOIN customer ON o_custkey = c_custkey GROUP BY c_name");
	ExpectRows(*result, {{Value::Varchar("alice"), Value::BigInt(2)}, {Value::Varchar("bob"), Value::BigInt(1)}}, true);
}
...
TEST(Lab3ExecutionTest, JoinLargeParallel) {
	...
	auto result = Run(db, "SELECT count(*) FROM lineitem JOIN keys ON l_orderkey = k");
...
TEST(Lab3ExecutionTest, OrderByGroupByResult) {
	...
	ExpectRows(*result, {{Value::Varchar("a"), Value::BigInt(6)},
...
TEST(Lab3ExecutionTest, ExpressionEvaluatorVarcharComparison) {
	...
	auto predicate = std::make_unique<BoundComparisonExpression>(
		ExpressionType::COMPARE_LESS_THAN, std::make_unique<BoundColumnRefExpression>("s", 0, LogicalType::Varchar()),
...
TEST(Lab3ExecutionTest, ExpressionEvaluatorConjunction) {
	...
	auto lt = std::make_unique<BoundComparisonExpression>(
		ExpressionType::COMPARE_LESS_THAN, std::make_unique<BoundColumnRefExpression>("a", 0, LogicalType::Integer()),
		std::make_unique<BoundConstantExpression>(Value::Integer(6)));
	// a > 2 AND a < 6 -> rows 3, 4, 5
	auto conjunction =
		std::make_unique<BoundConjunctionExpression>(ExpressionType::CONJUNCTION_AND, std::move(gt), std::move(lt));
	SelectionVector sel;
...
TEST(Lab3ExecutionTest, ProjectionConstantColumns) {
	...
	ExpectRows(*result, {{Value::Integer(42), Value::Varchar("x"), Value::Integer(1)},
...
TEST(Lab3ExecutionTest, OrderByMultipleKeys) {
	...
	ExpectRows(*result, {{Value::Integer(1), Value::Integer(2)},
